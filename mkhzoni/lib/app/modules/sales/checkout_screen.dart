  ...
  Future<void> _confirmSale() async {
    ...
    if (_saleType == SaleType.credit && _selectedClient == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('الرجاء اختيار عميل لإتمام البيع الآجل'),
    ...
    try {
      ...
      if (_saleType == SaleType.credit && _selectedClient != null) {
        final clientRef =
            firestore.collection('clients').doc(_selectedClient!.id);
        // زيادة إجمالي الدين على العميل بقيمة الفاتورة
        batch.update(
            clientRef, {'totalDebt': FieldValue.increment(widget.totalAmount)});
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('حدث خطأ فادح: $error'),
              backgroundColor: Colors.red),
        );
      }
    } finally {
